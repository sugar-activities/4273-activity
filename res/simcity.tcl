...
Because of the toroidal nature of the the SimCity Space/Time Continuom, your city has wrapped back in time to 1900!} \
{{view {PanView $v [sim CenterX] [sim CenterY]}}}
...
"Linux SimCity Version [sim Version] Copyright (C) 2007
    by Electronic Arts, Maxis Studios.
Based on the Original SimCity Concept and Design
    by Will Wright, Maxis Software, Electronic Arts.
...
option add Mwm*SimCity.clientDecoration -maximize
...
proc ShowSplashOf {head} {
  ...
  $splashscreen config -bitmap "@images/splashscreen.xpm"
  wm deiconify $win
...
proc UIStartSimCity {homedir resourcedir hostname} {
...
proc UIUpdateRunning {} {
  global HeadWindows Time State
	...
	if {($State == "play") && $Time} {
    set pict "@images/simcityg.xpm"
  } else {
    set pict "@images/simcitys.xpm"
  }

  foreach win $HeadWindows {
    set can [WindowLink $win.demand]
    $can itemconfig simcity -bitmap $pict
  }
}
